...
########### CHARACTERIZATION OF CATEGORIES DISTRIBUTIONS ###########

#average dataset

#MEASURES
hourly=df.groupby(['Hour']).mean()
hourlyav=hourly.iloc[:,hourly.columns.isin(k for k in hourly.columns if 'Average' in k)]
monthly=df.groupby(['month']).mean()
monthlyav=monthly.iloc[:,monthly.columns.isin(k for k in monthly.columns if 'Average' in k)]
dailyvariance=hourlyav.var()
seasonalvariance=monthlyav.var()
...
trend=decomptrend.trend.resample('H').mean()
